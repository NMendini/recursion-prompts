...
var exponent = function(base, exp) {
  ...
  if (exp < 0) {
    return parseFloat((((1 / base) * (exponent(base, exp + 1)) * 100) / 100).toFixed(5));
  }
};
...
if (string.slice(0, 1) === string.slice(string.length - 1)) {
  return palindrome(string.slice(1, string.slice(string.length -1)));
} else {
  return false;
}
};
...
var flatten = function(array) {
  ...
  // iterate over array
    // if current is array
      // recurse to self
    // else
      // push current to result variable
  // return result
